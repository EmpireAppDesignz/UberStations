//
//  HistoryStationViewController.m
//  JukeBox
//
...
@interface HistoryStationViewController ()

@end

@implementation HistoryStationViewController
...
- (void)viewDidLoad
{
    ...
    HistoryStation_SongIdArray=[[NSMutableArray alloc] init];
    HistoryStation_TitlenameArray =[[NSMutableArray alloc] init];
    HistoryStation_SubTitlenameArray=[[NSMutableArray alloc] init];
    HistoryStation_SongUrlArray =[[NSMutableArray alloc] init];
    HistoryStation_ImageUrlArray=[[NSMutableArray alloc] init];
...
-(void)GetData
{
    ...
    else
    {
        ...
        FMResultSet *results = [db executeQuery:[NSString stringWithFormat:@"select * from StationHistory"]];
        while([results next])
        {
            [HistoryStation_SongIdArray addObject:[NSString stringWithFormat:@"%d",[results intForColumn:@"stationid"]]];
            ...
            [HistoryStation_TitlenameArray addObject:ConcatName];
            ...
            [HistoryStation_SubTitlenameArray addObject:ConactSubName];
            
            
            [HistoryStation_SongUrlArray addObject:[NSString stringWithFormat:@"%@",[results stringForColumn:@"stationurl"]]];
            
            [HistoryStation_ImageUrlArray addObject:[NSString stringWithFormat:@"%@",[results stringForColumn:@"stationimageurl"]]];
        }
        NSLog(@"Result is:%@",results);
    }
    [HistoryStationTbl reloadData];
}
#pragma mark Tableview Methods
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    return HistoryStation_TitlenameArray.count;
}
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    nameLabel.text = [HistoryStation_TitlenameArray objectAtIndex:indexPath.row];
    ...
    SubnameLabel.text = [HistoryStation_SubTitlenameArray objectAtIndex:indexPath.row];
    // custom views should be added as subviews of the cell's contentView:
    ...
    FeedSearchUserImage.imageURL=[NSURL URLWithString:[HistoryStation_ImageUrlArray objectAtIndex:indexPath.row]];
...
{
    NSLog(@"SubName is:%@",[HistoryStation_SubTitlenameArray objectAtIndex:indexPath.row]);
    NSLog(@"Name is:%@",[HistoryStation_TitlenameArray objectAtIndex:indexPath.row]);
    
    app=[[UIApplication sharedApplication] delegate];
    app.bottomplayerview.currentSongIndex=0;
    app.bottomplayerview.SongSubName=[HistoryStation_SubTitlenameArray objectAtIndex:indexPath.row];
    app.bottomplayerview.currentSongsNameArray=[HistoryStation_TitlenameArray objectAtIndex:indexPath.row];
    app.bottomplayerview.currentSongsURLArray=[HistoryStation_SongUrlArray objectAtIndex:indexPath.row];
    app.bottomplayerview.currentSongsIdArray=[HistoryStation_SongIdArray objectAtIndex:indexPath.row];
    NSString *TempImg=@"Icon-40.png";
    app.bottomplayerview.ImageUrlStr=[HistoryStation_ImageUrlArray objectAtIndex:indexPath.row];
    ...
    if ([app.bottomplayerview.PlayerShowStr isEqualToString:@"Yes"])
    {
        ...
        {
            HistoryStationTbl.frame=CGRectMake(0, 46, 320, 485);
        }
        else
        {
            HistoryStationTbl.frame=CGRectMake(0, 46, 320, 395);
        }
    }
}
